...
#define ROC_DPI_MBOX_TIMEOUT_MS 1000
...
static inline int
send_mbox_cmd(struct roc_dpi *roc_dpi, dpi_mbox_msg_t *cmd, dpi_mbox_msg_t *rsp)
{
	volatile uint64_t reg_val;
	int count;

	plt_write64(cmd->u[0], roc_dpi->rbase + DPI_MBOX_VF_PF_DATA1);
	plt_write64(cmd->u[1], roc_dpi->rbase + DPI_MBOX_VF_PF_DATA0);

	/* No response for notification messages */
	if (!rsp)
		return 0;

	for (count = 0; count < ROC_DPI_MBOX_TIMEOUT_MS; count++) {
		rte_delay_ms(1);
		reg_val = plt_read64(roc_dpi->rbase + DPI_MBOX_VF_PF_DATA0);
		if (reg_val != cmd->u[1]) {
			rsp->u[0] = reg_val;
			break;
		}
	}
	if (count == ROC_DPI_MBOX_TIMEOUT_MS) {
		plt_err("DPI mbox cmd send timeout\n");
		return ROC_DPI_MBOX_TIMEOUT_MS;
	}
	if (rsp->u[0] != DPI_MBOX_TYPE_RSP_ACK) {
		plt_err("DPI mbox received  NACK from PF\n");
...
static inline int
send_msg_to_pf(struct roc_dpi *roc_dpi, dpi_mbox_msg_t *cmd, dpi_mbox_msg_t *rsp)
{
	int ret;

	rte_spinlock_lock(&roc_dpi->mbox_lock);
	ret = send_mbox_cmd(roc_dpi, cmd, rsp);
	rte_spinlock_unlock(&roc_dpi->mbox_lock);
...
{
	dpi_mbox_msg_t mbox_msg = { 0 }, rsp = { 0 };
	uint64_t reg;
	int rc;
	...
	rc = send_msg_to_pf(roc_dpi, &mbox_msg, &rsp);
	if (rc < 0)
...
roc_dpi_dev_init(struct roc_dpi *roc_dpi, uint8_t offset)
{
	...
	plt_spinlock_init(&roc_dpi->mbox_lock);
